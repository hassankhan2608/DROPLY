...
import { FiUser, FiKey, FiEye, FiEyeOff, FiLoader, FiChevronDown, FiAlertCircle } from 'react-icons/fi';

const CreateAccountForm = ({ formData, handleChange, handleSubmit, showPassword, setShowPassword, loading, domains, error }) => {
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <select
                ...
                onChange={handleChange}
                className="relative z-10 appearance-none w-full bg-zinc-800/50 border border-zinc-700/50 rounded-lg pl-3 pr-10 py-2.5 text-white focus:outline-none focus:border-green-500/50 transition-colors cursor-pointer hover:bg-zinc-800"
                required
              >
                {domains.map((domain) => (
                  <option key={domain.id} value={domain.domain} className="bg-zinc-900 text-white py-2">
                    @{domain.domain}
                  </option>
                ))}
              </select>
